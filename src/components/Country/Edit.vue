    ...
    <q-card-section class="q-pt-lg">
      <div class="row">
        <div class="col-12 col-md-12 q-pr-md">
          <q-input :autofocus="true" v-model="country.name"
      :label="$t(Utils.getKey('Name'))"
      :oninput="(evt) => Utils.containsOnlyCharacter2(evt)"
            :rules="[
              (val) =>
...
onMounted(() => {
  languages.value.forEach((lg) => {
    country.value.translation_name[lg.locale_web] = country.value?.translates[lg.locale_web]?.name || "";
  });
});
</script>
